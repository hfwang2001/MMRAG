...
class EdgeTTS(BaseTTS):
    def txt_to_audio(self, msg):
        ...
        # 检查是否有音频数据生成
        if self.input_stream.getbuffer().nbytes <= 0:  # edge-tts 出错
        ...
        streamlen = stream.shape[0]  # 音频样本总数
        
        # 计算音频时长（秒）
        audio_duration = streamlen / self.sample_rate  # sample_rate = 16000
        
        # 截取前1秒音频的样本数（若总时长>=1秒）
        samples_1s = min(self.sample_rate, streamlen)  # 最多取16000个样本（1秒）
        ...
        if audio_duration >= 1.0:
            rtf = processing_time / 1.0  # 目标是1秒音频
        else:
            rtf = processing_time / audio_duration  # 如果音频不足1秒，按实际时长算
        
        # 日志记录时间和RTF
        logger.info(f'-------Edge TTS processing time: {processing_time:.4f}s for {audio_duration:.4f}s audio, RTF: {rtf:.4f}')
    ...
    # __create_bytes_stream 和 __main 方法保持不变
...
class CosyVoiceTTS(BaseTTS):
    ...
    def stream_tts(self,audio_stream,msg):
        ...
        for chunk in audio_stream:
            if chunk is not None and len(chunk)>0:          
                stream = np.frombuffer(chunk, dtype=np.int16).astype(np.float32) / 32767
                stream = resampy.resample(x=stream, sr_orig=16000, sr_new=self.sample_rate)
                #byte_stream=BytesIO(buffer)
                #stream = self.__create_bytes_stream(byte_stream)
                streamlen = stream.shape[0]
                idx=0
